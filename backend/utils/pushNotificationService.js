...
async function sendExpoPushNotification(expoPushTokens, notification) {
  ...
  const messages = validTokens.map(token => ({
    ...
    priority: 'high',
    channelId: notification.channelId || notification.data?.channelId || 'default',
 ...
 */
export async function registerPushToken(userId, userType, expoPushToken, deviceType, deviceId) {
  try {
    // Validate expo token format
    if (!expoPushToken.startsWith('ExponentPushToken[') && !expoPushToken.startsWith('ExpoPushToken[')) {
      console.warn('Invalid Expo push token format:', expoPushToken);
      return { success: false, error: 'Invalid token format. Expected ExponentPushToken[xxx] format.' };
    }
...
export async function sendPushNotification(userId, notification) {
  try {
    ...
    if (error) {
      ...
      await logNotification(userId, 'unknown', notification, { success: false, error: 'DB error fetching tokens' });
      return { success: false, error };
    }

    if (!tokens || tokens.length === 0) {
      console.log(`❌ No active push tokens for user ${userId}`);
      await logNotification(userId, 'unknown', notification, { success: false, error: 'No tokens found' });
    ...
    await logNotification(userId, tokens[0]?.user_type || 'unknown', notification, result);
 ...
 */
export async function sendAdminApprovalNotification(adminUserId, restaurantName, isApproved = true) {
  try {
 ...
 * @param {boolean} isApproved - true = approved, false = rejected
 */
export async function sendDriverApprovalNotification(driverId, driverName, isApproved = true) {
  try {
    const notification = isApproved
      ? {
 ...
 */
export async function sendNewOrderNotification(restaurantId, orderInfo, adminIds = null) {
  try {
    console.log('📱 sendNewOrderNotification called for restaurant:', restaurantId, 'adminIds:', adminIds);
    ...
    if (!resolvedAdminIds || resolvedAdminIds.length === 0) {
      ...
      console.log('🔍 Found admins from admins table:', { admins, error, restaurantId });

      if (error || !admins || admins.length === 0) {
        console.log(`❌ No admins found for restaurant ${restaurantId}:`, error);
    ...
    const notification = {
      ...
      body: `Order #${orderInfo.orderNumber} — ${orderInfo.itemsCount} item(s) — Rs. ${Number(orderInfo.totalAmount).toFixed(2)}`,
      sound: 'default',
    ...
    console.log('📤 Sending push notifications to', resolvedAdminIds.length, 'admin(s)');
    ...
    console.log(`✅ Push: New order notification sent to ${resolvedAdminIds.length} admin(s)`);
...
export async function sendOrderStatusNotification(customerId, orderInfo) {
  try {
    ...
    if (!config) return { success: false, error: `Unknown status: ${orderInfo.status}` };
...
export async function sendDeliveryStatusNotification(customerId, info) {
  try {
    ...
    if (!config) return { success: false, error: `Unknown status: ${info.status}` };
 ...
 */
export async function sendMilestoneNotification(userId, userType, milestoneInfo) {
  try {
    console.log(`🎉 sendMilestoneNotification to ${userType} ${userId}:`, milestoneInfo);
    ...
    return await sendPushNotification(userId, {
      ...
      body: milestoneInfo.message || `Amazing! You've completed ${milestoneInfo.milestone} orders today!`,
      sound: 'default',
 ...
 */
export async function testPushNotification(userId, title = 'Test Notification', body = 'Push notifications are working!') {
  return await sendPushNotification(userId, {
    title,
    body,
