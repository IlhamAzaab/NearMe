...
export default function AdminRestaurantPending() {
  ...
  const loadStatus = async () => {
    try {
      const res = await fetch(
        `${API_URL}/restaurant-onboarding/status`,
        {
      ...
      if (res.ok) {
        ...
        if (data.restaurant && data.restaurant.restaurant_status === 'Active') {
          setTimeout(() => {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center bg-gradient-to-br from-green-500 via-green-600 to-green-700 p-4 overflow-hidden relative transition-transform duration-500 ease-in-out ${isTransitioning ? '-translate-x-full opacity-0' : 'translate-x-0 opacity-100'}`}>
      ...
      <div className="max-w-xl w-full mx-auto bg-white/95 backdrop-blur-sm rounded-3xl shadow-2xl p-8 text-center relative z-10">
        ...
        <h1 className="text-3xl font-bold mb-4 bg-gradient-to-r from-lime-600 to-green-600 bg-clip-text text-transparent">
        ...
        <div className="w-full max-w-md mb-8">
          <div className="relative h-3 bg-gray-200 rounded-full overflow-hidden shadow-inner">
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-green-500 to-transparent animate-scan-line"></div>
          </div>
        </div>
        ...
        {loading && (
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-sm text-gray-500 font-medium">Loading status...</p>
          </div>
        )}
        {!loading && status && status.restaurant && (
          <div className="space-y-4">
            <div className={`bg-gradient-to-r ${status.restaurant.restaurant_status === 'Active' ? 'from-green-100 to-green-200 border-green-400' : 'from-green-50 to-green-100 border-green-200'} border-2 rounded-xl p-6 text-center shadow-lg transition-all duration-500`}>
              <p className="flex justify-center items-center gap-3">
                <span className="text-lg font-semibold text-gray-800">Restaurant Status:</span>
                <span className={`bg-white px-4 py-2 rounded-lg font-semibold shadow-sm text-lg ${status.restaurant.restaurant_status === 'Active' ? 'text-green-700 ring-2 ring-green-500' : 'text-green-600'}`}>
                  {status.restaurant.restaurant_status}
                </span>
              </p>
              {status.restaurant.restaurant_status === 'Active' && (
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-semibold py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 flex items-center justify-center gap-2"
            >
